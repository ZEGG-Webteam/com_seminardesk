...
use Joomla\Component\Seminardesk\Site\Controller\ApiController;
...
//      self::LABELS_FESTIVALS_ID, 
  ...
  public static function getApiController()
  {
    if (!self::$api_controller) {
      self::$api_controller = new ApiController();
    }
  ...
  {
    $config = self::getConfiguration();
    return Route::_($config['facilitators_base'] . "&view=facilitator&id=" . $facilitator->id . '&name=' . self::createSlug($facilitator->name));
  }
  ...
  public static function loadFacilitator($facilitatorId)
  {
    ...
    if (is_object($data) && $data) {
      ...
      // Error handling
      Log::add(
        'loadFacilitator('.$facilitatorId.') failed! ($facilitatorData = ' . json_encode($data) . ')', 
        Log::ERROR, 
        'com_seminardesk'
      );
      $facilitator = [];
...
//    if ($eventDate->categoriesList)     { $classes[] = 'has-categories';   } // Hide categories in List for now
    ...
    //-- URLs
    $eventDate->detailsUrl = ($eventDate->detailpageAvailable)?self::getEventUrl($eventDate):'';
    $eventDate->bookingUrl = self::getBookingUrl($eventDate->eventId, $eventDate->titleSlug, $eventDate->id);
  }
  ...
  public static function prepareEvent(&$event)
  {
    ...
    $event->bookingUrl = self::getBookingUrl($event->id, $event->titleSlug);
    ...
    foreach($event->dates as $key => $date) {
      ...
      $date->dateFormatted = self::getDateFormatted($date->beginDate, $date->endDate);
...
//      }
      ...
      $date->bookingUrl = self::getBookingUrl($event->id, $event->titleSlug, $date->id);
      $date->statusLabel = self::getStatusLabel($date);
  ...
  public static function prepareFacilitator(&$facilitator)
  {
    ...
    $facilitator->detailsUrl = self::getFacilitatorUrl($facilitator);
...
\class_alias(DataHelper::class, 'SeminardeskDataHelper');
